...
class PubTracker(object):
  def __init__(self, max_age=0, max_dist={}, score_thresh=0.1):
    ...
    self.KITTI_CLS_VELOCITY_ERROR = max_dist 

    self.KITTI_TRACKING_NAMES = KITTI_TRACKING_NAMES
  ...
  def step_centertrack(self, results, time_lag):
    if len(results) == 0:
      ...
      for det in results:
        ...
        if det['detection_name'] not in self.KITTI_TRACKING_NAMES:
          print("filter {}".format(det['detection_name']))
          continue 

        det['ct'] = np.array(det['translation'][:2])
        det['tracking'] = np.array(det['velocity'][:2]) * -1 *  time_lag
        det['label_preds'] = self.KITTI_TRACKING_NAMES.index(det['detection_name'])
        temp.append(det)
    ...
    max_diff = np.array([self.KITTI_CLS_VELOCITY_ERROR[box['detection_name']] for box in results], np.float32)
