...
def boxes3d_lidar_to_fake_kitti_camera(boxes3d_lidar):
    """
    :param boxes3d_lidar: (N, 7) [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
    :param calib:
    :return:
    ...
    # xyz_lidar[:, 2] -= h.reshape(-1) / 2
    xyz_cam = np.zeros((xyz_lidar.shape[0], 3))
    xyz_cam[:,0] = -xyz_lidar[:, 1]  # x = -y_lidar
    xyz_cam[:,1] = -xyz_lidar[:, 2]  # y = -z_lidar
    xyz_cam[:,2] = xyz_lidar[:, 0]  # z = x_lidar
    r = -r - np.pi / 2
    return np.concatenate([xyz_cam, l, h, w, r], axis=-1)

def boxes3d_fake_kitti_camera_to_lidar(boxes3d_camera):
    """
    Args:
        boxes3d_camera: (N, 7) [x, y, z, l, h, w, r] in rect camera coords
        calib:
    ...
    xyz_lidar = np.zeros((xyz_camera.shape[0], 3))
    xyz_lidar[:,0] = xyz_camera[:, 2]  # z = x_lidar
    xyz_lidar[:,1] = -xyz_camera[:, 0]  # x = -y_lidar
    xyz_lidar[:,2] = -xyz_camera[:, 1]  # y = -z_lidar
